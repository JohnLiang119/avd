...
content = content.replace(/const store = new Store\('config\.json'\);\s*/, `${saveConfigCode}\n`);

const initStoreRegex = /const initStore = async \(\) => \{[\s\S]*?isStoreInitialized\.value = true;\n\};/m;

const newInitStoreCode = `const initStore = async () => {
  if (!isTauri()) {
...
content = content.replace(initStoreRegex, newInitStoreCode);

content = content.replace(/watch\(monitoredChannels, async \(val\) => \{[\s\S]*?\}, \{ deep: true \}\);/m, `watch(monitoredChannels, (val) => {
  if (!isStoreInitialized.value) return;
  saveConfig('avd_monitored_channels', val);
}, { deep: true });`);

content = content.replace(/watch\(monitorConfig, async \(val\) => \{[\s\S]*?\}, \{ deep: true \}\);/m, `watch(monitorConfig, (val) => {
  if (!isStoreInitialized.value) return;
...
content = content.replace(/localStorage\.setItem\('avd_monitored_channels', JSON\.stringify\(val\)\);/g, "saveConfig('avd_monitored_channels', val);");
content = content.replace(/localStorage\.setItem\('avd_monitor_config', JSON\.stringify\(val\)\);/g, "saveConfig('avd_monitor_config', val);");
...
console.log('App.vue patched successfully.');
