...
當 `all.ps1` 執行 Windows 版打包（`tauri build`）遭遇失敗時，系統MUST（必須）自動顯示警告訊息、自動清理 `src-tauri\target` 快取目錄，並自動進行第二次全新打包嘗試。
...
`all.ps1` 腳本MUST（必須）支援 `-Clean` 參數，允許使用者手動指定在編譯前先行清理 `src-tauri\target` 快取。
