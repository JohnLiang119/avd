...
發布腳本執行時，系統MUST（必須）自動從專案配置中讀取版本號，並檢查相應版本的二進制安裝檔是否存在。
...
在發布 Release 之前，腳本MUST（必須）確保本機的程式碼變更均已提交並推送到 GitHub 遠端倉庫。
...
系統MUST（必須）透過 GitHub CLI 自動建立 Release，並將 APK 與 MSI 附件上傳。
