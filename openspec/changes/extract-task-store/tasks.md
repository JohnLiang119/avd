...
- [x] 6.1 手動驗證五條 UI 路徑的響應性：新增任務、進度更新、群組展開、刪除任務、清除紀錄
  - 由使用者於 v1.0.64 實機驗證，回報全數正常。
  - 此項涵蓋自動化測試無法觸及的部分：`defineSetting` 回傳的 `Ref` 在模板中的響應性，以及 `hydrate()` 與 `onMounted` 的實際執行時序。
- [x] 6.2 Windows 端驗證中斷清理：下載中強制關閉 app → 重啟後任務為 `error` 且可重試 → 再次關閉重啟後仍為 `error`
  - 由使用者於 v1.0.64 實機驗證，回報行為正確。
  - 這是本變更的核心修正 —— 舊版此處會被 Tauri Store 的還原覆蓋，使任務永遠卡在「下載中」。
  - 同時完成舊格式相容性的真實驗證：首次啟動會從既有的 Tauri Store 讀入使用者實際累積的任務資料。
- [x] 6.3 Android 端驗證同一情境行為一致
  - 由使用者於 v1.0.64 實機驗證，回報與 Windows 端行為一致。
- [x] 6.4 量測改善幅度
