...
元件置於既有「重整、清除、刪除、設定」與「音訊、頻道、快傳、收合」兩排控制按鈕最左側，高度與該兩排等高（垂直置中）；兩排按鈕仍維持原本 `justify-content: flex-end` 靠右對齊。`online` 僅顯示文字「網路正常」，不加色點或圖示——狀態文字本身已足夠清楚，不構成僅靠顏色傳達狀態的疑慮；`checking` 保留小型轉圈動畫與文字；`degraded`／`offline` 則在這兩排按鈕正上方展開全寬橘色／紅色提示列並顯示重新檢查按鈕，不與按鈕擠在同一排，此二態仍併用圖示與文字。

版面歷經兩次調整：最初放在網址輸入列正上方、獨立佔一整列；第二版改為僅與「重整」單排同高、置左；使用者檢視後要求進一步改為與「重整」「音訊」兩排等高（視覺上更像一個獨立的左側狀態欄），並移除 `online` 狀態原有的綠色圓點圖示，只保留文字。
