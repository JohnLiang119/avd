...
### 4. 正常狀態收斂為標籤，異常狀態才展開；併入「重整／音訊」兩排控制列而非獨立一列

元件置於既有「重整、清除、刪除、設定」與「音訊、頻道、快傳、收合」兩排控制按鈕最左側，高度與該兩排等高（垂直置中）；兩排按鈕仍維持原本 `justify-content: flex-end` 靠右對齊。`online`／`checking` 的標籤文字改為每兩字一行堆疊呈現（如「網路」／「正常」），讓標籤本身的視覺高度也貼近兩排按鈕，而非在高聳的置中容器內只浮著一顆單行小圓標籤。`online` 不加色點或圖示——堆疊後的文字本身已足夠清楚，不構成僅靠顏色傳達狀態的疑慮；`checking` 保留小型轉圈動畫，置於堆疊文字上方；`degraded`／`offline` 則在這兩排按鈕正上方展開全寬橘色／紅色提示列並顯示重新檢查按鈕，不與按鈕擠在同一排，此二態仍併用圖示與文字（維持單行呈現，不套用堆疊）。

版面歷經三次調整：最初放在網址輸入列正上方、獨立佔一整列；第二版改為僅與「重整」單排同高、置左；第三版改為與「重整」「音訊」兩排等高並移除 `online` 的綠色圓點圖示；使用者檢視第三版後認為標籤文字仍集中在容器中段、視覺上沒有真正「佔滿」兩排，因此再改為文字每兩字一行堆疊（如「網路」／「正常」）以撐滿高度。
...
- **[Risk] 主畫面垂直空間更擁擠。** → 正常／檢查中狀態不新增一列，直接併入既有「重整」控制列最左側；只有需要處理的異常狀態才在該列上方展開完整提示列。
