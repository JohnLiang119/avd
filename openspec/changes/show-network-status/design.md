...
### 4. 正常狀態收斂為標籤，異常狀態才展開；併入「重整」控制列而非獨立一列

元件與既有「重整、清除、刪除、設定」控制按鈕列同排：`online` 與 `checking` 使用精簡標籤，緊貼該列最左側，按鈕仍維持原本 `justify-content: flex-end` 靠右對齊；`degraded`／`offline` 則在該按鈕列正上方展開全寬橘色／紅色提示列並顯示重新檢查按鈕，不與按鈕擠在同一排。所有狀態同時使用圖示與文字，避免只靠顏色。

最初版本曾將元件放在網址輸入列正上方、獨立佔一整列；使用者檢視後認為與「重整／音訊」控制列的視覺關聯不足，改為併入該控制列最左側，縮短視覺掃描距離並減少多一列造成的垂直空間占用。
