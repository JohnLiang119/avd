...
- 通知標題用 `channel.name`：file 頻道的名稱預設為第一個檔案的顯示名稱（多檔時加「等 N 個檔案」），使用者可改名。
- **這是第一個「新增頻道」的介面**。規格原寫「MAY 不提供新增自訂頻道的介面」，加了 file 就非做不可：頻道列多一顆新增（開選檔器），非內建頻道可展開看清單、改名、移除；移除時刪掉私有目錄下該頻道的資料夾，指向它的鬧鐘落回預設頻道（既有邏輯）。自訂串流網址（url kind）的新增介面**仍不做**，維持只由舊資料遷移產生。
