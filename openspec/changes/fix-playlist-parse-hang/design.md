...
- ~~**Android 上「數分鐘」的組成尚未拆解。**~~ **已解答（2026-09-04，經 v1.0.71 的錯誤紀錄取得原文）。**

  兩塊各自成立，且原先並列的「兩個候選」其實是同一件事：

  1. **等待來自枚舉**。Windows 端全抓 3247 筆需 2 分 18 秒；加上 200 筆上限後首批 11 秒。Android 實機（任務 10.11）連續三批 1-200／201-400／401-600 皆在可接受時間內完成，證實此塊已解決。
  2. **失敗來自 TikTok 限流**，而非 extractor 缺陷。原先並列的兩個候選 —— `Unable to extract secondary user ID` 與 HTTP 429 —— 是**同一現象的兩張臉**：TikTok 擋下請求時回一個不含 `sec_uid` 的頁面，yt-dlp 只能報「抽不出來」。

  判定依據（同日五個資料點）：官方 `@tiktok` 帳號也中過（不可能是私人）；同一帳號稍後重試成功 3247 筆；`@ttggwang` 在 Android 失敗的同時段自 Windows（另一 IP）打完全正常。yt-dlp 併發的「account is either private or has embedding disabled」警告對公開帳號一樣出現，是同一個被擋頁面的產物，不可據以判斷。

  處置已於 `source-profile-registry` 的第 7 組實作（v1.0.72 擋明確狀態碼、v1.0.73 補上間接徵狀），兩者皆退避重試並改寫為可行動的訊息。本變更的設計不需因此調整 —— D3 的時間上限與數量上限對「慢成功」與「慢失敗」同樣有效，此點原判斷成立。
