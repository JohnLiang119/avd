...
> **驗證範圍的決定（2026-09-10）**：5.1～5.5 的五項建置驗證已實際執行並通過。
> 5.6～5.12 的**人工驗證經使用者決定不逐項執行**，改以正常使用中發現問題再回報。
>
> 因此下列行為**僅有單元測試覆蓋、未經實機驗證**，是本 change 已知的驗證缺口：
> 檢查迴圈的實際佈線、關鍵字編輯 UI、備份還原的四種輸入、兩個模擬入口、
> 以及回饋文案在真實情境下的呈現。純函式層（正規化、字形折算、輸入上限、
> 錨點守門、訊息片段）有 56 個單元測試覆蓋，不在此缺口內。
>
> 風險已由使用者評估並接受。
...
- [x] 5.6 人工驗證【基本篩選】：無關鍵字頻道維持全量追蹤；設定單一與多重關鍵字時 OR 命中；未命中影片不出現在下載佇列；清空所有關鍵字後恢復全量追蹤；修改關鍵字不回補先前已檢查的影片
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.7 人工驗證【D-C 直播查詢未發出】：對設有關鍵字的頻道執行立即檢查，於開發者工具 Console／錯誤日誌確認**未命中影片沒有任何直播狀態查詢紀錄**，僅命中影片有
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.8 人工驗證【D-B 錨點守門】：構造「較新的未命中影片 + 較舊的命中但屬直播／狀態未知影片」情境，確認該直播影片在下一輪仍被視為新片（錨點未越過它），且未命中影片不再重複比對
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.9 人工驗證【D-G 回饋訊息】：分別造出「本輪無任何新片」與「本輪有新片但全部未命中關鍵字」，確認兩者提示文案明確不同；並確認「發現 N 部新影片」的 N 等於佇列實際新增的任務數（命中但因直播跳過者不計入）
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.10 人工驗證【備份還原】：匯出含關鍵字的備份並檢視 JSON；以覆蓋與合併兩模式還原新版備份、缺欄位的舊版備份、含非字串／空白／重複值的備份；確認【D-H】備份錨點較新時保留本機較舊值、合併時因 Channel ID 已存在而未套用關鍵字的頻道數出現在提示中
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.11 人工驗證【UI 與持久化】：桌面與手機寬度下頻道卡片維持雙行、無橫向捲軸；關鍵字對話框的 Enter／半形逗號／全形逗號、單項刪除、全部清空、取消不寫入皆正常；【D-E】超出 20 項或 100 字元時有拒絕提示且未靜默截斷、逗號不可作為關鍵字內容的說明可見；重新啟動應用程式後關鍵字仍存在
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 5.12 人工驗證【單／全頻道模擬】：兩個模擬入口皆只選命中影片（全頻道每頻道最多兩支），無命中時顯示專用提示且不建立任何測試任務
  - **未逐項執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以實際使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
