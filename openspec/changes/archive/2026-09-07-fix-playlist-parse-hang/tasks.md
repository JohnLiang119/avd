...
- [x] 4.4 確認 YouTube 既有的兩段式流程（加入追蹤 → 掃描明細）未受影響
...
- [x] 6.4 Android 實機：同一個 TikTok 網址，確認等待時間由數分鐘壓至可接受範圍，並記錄失敗訊息原文（回填 design 的 Open Question）
  - **等待時間**：由任務 10.11 覆蓋 —— 連續三批 1-200／201-400／401-600 皆完成，
    時間若不可接受根本抓不完（每批受 90 秒上限約束，逾時即中止）。
  - **失敗訊息原文**：由 v1.0.71 的錯誤紀錄取得兩則實錄 ——
    `HTTP Error 429: Too Many Requests`（17:15）與
...
- [x] 6.7 Android 實機：既有的 YouTube 頻道與播放清單批次下載流程完整走一遍，確認未回歸
...
- [x] 8.6 記錄：`/channel/{id}` 的頂層回傳 3 筆 `_type: playlist`（Videos 117／
      ...
      已併入 `source-profile-registry` 任務 3.6 一併解決（實測確認
      `--playlist-end 200` 打在頻道網址上時，內嵌 entries 已各自裁到 200，
      與逐分頁呼叫結果相同，首批可直接採用而省下 3 次呼叫）。

- [x] 8.7 **⑤ 對多序列來源會永久漏片**（本變更交付後發現，已併入
      `source-profile-registry` 第 3 組）。單位不一致：進度是全域計數，
      批次範圍卻套用在每個分頁上。以 Lofi Girl 頻道實測 ——
      首批 `--playlist-end 200` 得 117+23+200 = 340 筆，進度推進為 340；
      第二批送 `--playlist-items 341-540`，三個分頁全部回傳 0 筆並被標記
      為已抓完，shorts 剩餘的 132 部永遠取不到。直接打 shorts 分頁驗證：
      `201-400` 得 132 筆（正解）、`341-540` 得 0 筆（現行行為）。
      單序列來源（`list=`、TikTok）不受影響，故任務 10.11 的 TikTok 驗證
      會通過而漏掉此缺陷 —— 實機驗證時無須為此加測。
...
- [x] 10.11 實機驗證：同一 TikTok 網址連續送出三次，確認抓到 1-200、201-400、
      401-600，且第二、三次的網址帶不同追蹤參數仍被視為同一來源
- [x] 10.12 實機驗證：對已有進度的來源選擇「從頭開始」，確認自第一批重抓
