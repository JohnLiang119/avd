...
- [x] 4.3 手動驗證 A：於 Android 上對含進行中直播的頻道執行手動檢查，確認直播未被加入佇列
  - 由使用者於 2026-09-09（v1.0.84）實機驗證通過：進行中的直播未進入下載佇列。
- [x] 4.4 手動驗證 B：直播被跳過後，確認該頻道卡片顯示的時間未推進至該直播的發布時間
  - 由使用者於 2026-09-09（v1.0.84）實機驗證通過：錨點未越過被跳過的直播影片。
...
- [x] 4.6 手動驗證 D：於 Android 分享或手動加入一支影片，確認任務標題顯示的發布時間為實際時間而非當日午夜
  - 由使用者於 2026-09-09（v1.0.84）實機驗證通過：任務標題顯示實際發布時間，非當日午夜。
