...
- **WHEN** 使用者執行 `release_avd.ps1` 且目錄中存在當前版號（例如 1.0.1）的 APK 與 MSI 檔案
- **THEN** 系統解析安裝檔路徑並顯示檔案大小，準備進行上傳

#### Scenario: 安裝檔缺失提示
- **WHEN** 使用者執行發布腳本但目錄中未找到對應版號的安裝包
- **THEN** 系統顯示友善錯誤提示，引導使用者先執行編譯腳本（`all.ps1`）
...
- **WHEN** 執行發布且遠端無同名 Tag
- **THEN** 系統呼叫 `gh release create <Tag>` 上傳 APK 與 MSI，並設定為最新發布（`--latest`）
