...
- [x] 4.1 開工前確認基線：`npx vitest run src/composables/__tests__/useChannelMatching.spec.ts` 的 56 個案例全數綠燈；完成方式：測試輸出顯示 56 passed（若非 56，先確認 keyword change 已歸檔且未被改動）
  - 59 個案例綠燈（任務原寫 56，3.4 的視窗規則修正已增至 59）。
- [x] 4.2 【D-I】將檢查迴圈的直播判定改為兩段式：先收集通過關鍵字篩選的候選影片，一次呼叫 `resolveLiveStatuses(videos)` 取得 `Map<videoId, LiveCheckResult>`，再走訪候選建立任務；該函式在 API 可用時走 2.5／2.7 的批次，否則退回既有逐支 `checkVideoLiveStatus`；完成方式：`grep -n "checkVideoLiveStatus\|resolveLiveStatuses" src/App.vue` 呈現迴圈內**不再**有 `await checkVideoLiveStatus`，且該呼叫只出現在 `resolveLiveStatuses` 的逐支實作中
  - `App.vue` 迴圈內不再有 `await checkVideoLiveStatus`；唯一呼叫點在 `DownloadService.resolveLiveStatuses` 的逐支降級路徑（`DownloadService.ts:1508`）。
  - **實作階段補一個缺口**：批次失敗原本被 `resolveLiveStatusesViaApi` 內部吞掉成 `unknown`，配額耗盡與金鑰無效將永遠不會觸發抑制、每輪重打必然失敗的請求。已加 `onBatchError` 上報，並由 `resolveLiveStatuses` 記錄抑制後**降級逐支完成本輪**（而非讓那些影片停在 unknown 白做一輪）。已補測試。
  - 一併把 `ChannelApiOptions` 與計數包裝抽為共用，避免兩個方法各寫一份而漂移。
- [x] 4.3 確認關鍵字收斂與未處理集合的語意未變：未命中關鍵字的影片 MUST NOT 進入批次查詢、MUST NOT 列入未處理集合；命中但狀態為 `'live'` 或 `'unknown'` 者仍列入未處理集合；完成方式：`grep -n "partitionByKeywords\|unhandledVideoIds" -A 4 src/App.vue` 呈現分割仍在直播判定之前、未處理集合只收命中影片，並由 6.7 人工驗證錨點守門行為不變
  - `partitionByKeywords` 仍在直播判定之前；只有 `matched` 進入 `resolveLiveStatuses`，未命中影片不觸發任何查詢；`unhandledVideoIds.add` 只在命中迴圈內。
- [x] 4.4 重跑既有測試確認未回歸；完成方式：`npm test` 全數通過，且 `useChannelMatching.spec.ts` 的案例數不減（4.1 的基線加上 3.4 新增者）
  - `npm test` 311 個測試全數通過（原 309）；vue-tsc、build、cargo check、gradlew 皆通過。
...
> **驗證範圍的決定（2026-09-10）**：6.1 的五項建置驗證已實際執行並通過。
> 6.2～6.11 的**人工驗證經使用者決定不逐項執行**，改以正常使用中發現問題再回報。
>
> 例外：6.6 屬安全關鍵，其核心（Rust 端 query string 遮蔽）已改為自動測試，
> 不單靠人工驗證。
>
> 已知的驗證缺口：未設金鑰時的實機行為、配額耗盡與金鑰無效的真實降級路徑、
> 設定 UI 的實際操作、批次查詢在真實資料上的生效、Android 端的 CORS 假設
> （design 決策 D-C 的關鍵前提，僅有 OPTIONS 預檢的實測佐證）、候選視窗
> 50 筆的真實情境、以及用量估算與 Console 的對照。
>
> 風險已由使用者評估並接受。
...
- [x] 6.2 人工驗證【未設金鑰行為不變】：清空金鑰後執行立即檢查，於開發者工具 Network 確認**沒有任何 `googleapis.com` 請求**，且行為與導入本 change 前一致（無錯誤、無要求設定金鑰的提示）
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.3 人工驗證【配額耗盡降級與抑制】：以額度已用盡的專案金鑰（或暫時撤銷配額）觸發，確認①該頻道降級至 RSS②本輪其餘頻道不再發出 API 請求③**重啟應用程式後仍不發出** API 請求④回饋明確告知已降級而非「沒有新影片」
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.4 人工驗證【金鑰無效】：填入一把無效金鑰，確認①提示可辨識為金鑰問題且與配額耗盡文案不同②降級至 RSS 後功能正常③不每頻道每輪重複提示④更換為有效金鑰後下次檢查恢復走 API
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.5 人工驗證【設定與機密】：金鑰輸入、已設定狀態指示、未預設明文顯示、說明文字；匯出頻道備份並檢視 JSON **不含金鑰**；還原備份後本機金鑰未被覆寫或清除；任務 `line` 顯示 API 來源標記
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.6 人工驗證【機密不入日誌】（安全關鍵）：分別造出 API 請求失敗與 RSS 請求失敗，開啟錯誤日誌並**完整檢視每一筆**，確認①無任何金鑰片段②RSS 失敗訊息不再含 `?channel_id=...` query string③仍能自日誌的頻道名稱上下文辨認是哪個頻道失敗
  - **部分改為自動測試（安全關鍵，不單靠人工）。** 原本無自動覆蓋的 Rust 端 query string 遮蔽已抽為
    `redact_query` 自由函式並加 5 個單元測試（`cargo test redact_tests`）：一般 query 移除、
    即使金鑰被放回網址也不外流、RSS 的 `?channel_id=` 移除但主機路徑保留、無 query 時不變、
    訊息中多處出現皆移除。
  - 金鑰走 `X-goog-api-key` 標頭而非網址，是結構性防護（`youtubeDataApi.spec.ts` 斷言網址不含金鑰任何片段）；
    回饋文案不接受金鑰參數，亦有測試。
  - **仍未涵蓋**：真實執行時錯誤日誌的實際內容、以及 Android 端 `fetch()` 路徑的錯誤訊息組成。
- [x] 6.7 人工驗證【錨點與關鍵字未回歸】：構造「較新的未命中關鍵字影片 + 較舊的命中但屬直播影片」，確認該直播影片下一輪仍被視為新片（錨點未越過它）；並確認設有關鍵字的頻道，未命中影片**沒有**進入批次查詢（Network 中批次請求的 `id` 參數不含該影片）
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.8 人工驗證【批次生效】：對設有金鑰且有多支新片的頻道執行檢查，於 Network 確認直播狀態查詢為**單一批次請求**（而非逐支多次），且 `id` 參數含多個以逗號分隔的影片識別碼
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.9 人工驗證【Android 端】：於 Android 實機重複 6.2、6.5、6.8，確認 WebView 的 `fetch()` 未被 CORS 阻擋（【D-C】的關鍵假設）
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.10 人工驗證【候選視窗 50 筆】：對上傳影片超過 50 支且長期未檢查的頻道執行檢查，確認錨點未跨過本輪未取回的較舊影片（下一輪仍能發現更舊的新片）
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。
- [x] 6.11 人工驗證【用量估算】：填入金鑰後執行數輪檢查，確認①對話框顯示的已用單位隨檢查次數增加②重啟應用程式後數值不歸零③與 Google Cloud Console「配額和系統限制」的實際數字比對，差距應可由「多裝置共用同一金鑰」或「同專案被其他工具使用」解釋 —— 若 app 的數字明顯**高於** Console，代表有多餘的呼叫（例如不慎分頁），須回報
  - **未執行。** 2026-09-10 使用者決定不做逐項人工驗證，改以正常使用中遇到問題再回報。
    此標記代表「已決定不執行」，**不代表已驗證通過**。

## 7. 提交與版本進版

- [x] 7.1 建立功能修正 commit，並以 `git status --short` 與 commit diff 確認只包含本 change 的程式、測試及 OpenSpec 任務進度
  - commit 含第 4 節的兩段式改造、`resolveLiveStatuses` 抽象、批次錯誤上報，以及 6.6 的 Rust 遮蔽單元測試。
