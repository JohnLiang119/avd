...
- **配額用量是否需要在 UI 呈現？** 原顧慮是 API 不提供查詢自身用量的端點 —— Console 上的數字走
  Service Usage／Cloud Monitoring，需 OAuth 的 `cloud-platform` 權限，非 API 金鑰可得，故 app 只能自行
  累計，而估算與實際可能不符（同專案被其他工具使用、多裝置共用同一把金鑰、失敗請求是否計費不確定）。

  **決議：併入本 change，並明確標示為估算值。** 實作成本低（`nextQuotaResetTime()` 已寫好且測過含日光
  節約時間邊界，`ChannelMonitorConfig` 已有選填欄位模式），且具額外的驗證價值 —— app 的估算值與 Console
  的真實值對照，可反過來檢查是否有多餘的呼叫（例如不慎分頁）。每分鐘配額刻意不顯示：其為專案層級數百萬
  單位，與本應用程式的用量差好幾個數量級，顯示只是版面雜訊。

  計數時機取「請求已送達服務」：傳輸層錯誤（`NETWORK_ERROR:`）未達 Google 故不計入，其餘（含配額耗盡的
  403）一律計入 —— 那些請求確實被服務處理過。
