...
- [x] 4.6 【紅線】確認一輪**正常**檢查不產生任何新日誌紀錄；完成方式：由 5.3 人工驗證 —— 檢查前後比對日誌筆數不變
...
- [x] 5.2 人工驗證【遮罩未改變擷取結果】：於實機對同一頻道檢查，確認取回的影片筆數、標題與發布時間皆與遮罩前一致；**特別確認發布時間不是 0 或當下時間** —— 那正是遮罩涵蓋不足的表徵

  實測確認影片筆數、標題與發布時間皆正常，**發布時間不是 0 也不是當下時間**
  —— 那正是遮罩涵蓋不足的表徵，未出現即代表遮罩完整。
- [x] 5.3 人工驗證【進度與日誌】：按下「立即檢查」，確認進度顯示已完成數／總數／當前頻道名稱且隨頻道推進、結束後不殘留；並確認該輪正常檢查前後的日誌筆數不變
- [x] 5.4 人工驗證【Android 實機速度】：記錄按下「立即檢查」到結果回饋出現的實際耗時，以及進度停在各頻道的時間；**此數據即 design 兩個 Open Question 的判斷依據**，於本檔記錄

  **實測（v1.0.93，Android）：19 個頻道、整輪約 6 秒**，即每頻道約 316 毫秒。
  使用者回報此速度可接受。

  **此數據回答了 design 的兩個 Open Question，兩者皆判定不做：**

  - **並行擷取**：6 秒已在可接受範圍，並行頂多再省數秒，卻要重構整個檢查
    迴圈並重新設計「提早停止」與「中途停擺」的語意，還會使進度回饋失去
    指認慢頻道的能力（並行只能說「已完成 7/19」）。不值得。
  - **以原生外掛消除 CORS 預檢**：每頻道 316 毫秒已包含預檢的那一趟往返。
    消掉它頂多省一半，換來的是新增平台層程式碼，且與 `api-only-channel-tracking`
    剛移除 Android 頻道方法的方向相反。不值得。

  兩者維持 Non-goal，且**現在有數據支撐而非推測**。
- [x] 5.5 人工驗證【逾時】：造出一個不會回應的情境（例如檢查中切斷網路），確認該頻道在 10 秒內被中止、進度繼續推進至下一個頻道、且 MUST NOT 出現「裝置目前無法連線」而中止整輪

  **通過，且序列完整證明兩個機制各走各的路**（實測 v1.0.97，檢查中切斷網路）：

  ```
    17:28:06  谷阿莫爽發就發   REQUEST_TIMEOUT:10000ms
              -> 逾時於 10 秒整觸發，且【未】中止整輪
    17:28:07  羅振宇           Failed to fetch
              -> 下一個頻道照常被檢查，進度確實推進
    17:28:07  提早結束，尚有 15 個頻道未檢查（裝置目前無法連線）
              -> 真離線才觸發提早結束
  ```

  第一個請求是斷線**之前**發出的，吊住直到 `AbortController` 於 10 秒砍掉；
  第二個是斷線**之後**發的，WebView 立刻拋 `TypeError: Failed to fetch`。

  若【D-C】的紅線沒守住（逾時被 `isDeviceOfflineError` 判為真），整輪會停在
  谷阿莫那裡並宣稱「尚有 16 個頻道未檢查」—— 實際停在 15 個且中間多跑了一個
  頻道，證明逾時走的是 `failedCount++` 而非 `break`。
- [x] 5.6 人工驗證【停擺入帳】：清空金鑰後等待自動排程反覆到期數次，確認日誌只有**一筆**停擺紀錄而非每分鐘一筆；填回金鑰後確認出現一筆解除紀錄

  **驗證不通過，已修（v1.0.94 之後）。** 實機回報：未設金鑰時日誌一口氣出現
  **28 筆同一秒、同一原因**的紀錄，context 為「模擬抓取頻道「…」」。

  根因不在自動排程（那條路徑的「只記轉換」運作正常），而在**兩個模擬入口
  沒有前置的追蹤狀態檢查**。`checkAllMonitoredChannels` 會在進迴圈前擋下
  並只提示一次，但 `simulateGlobalNewVideo` 會逐一走訪每個頻道、每個都拋出
  同一個停擺錯誤、每個都經任務 4.5 新增的入帳寫一筆 —— 20 個頻道就是
  20 筆一模一樣的紀錄，把只保留 50 筆的日誌吃掉大半。這正是
  `error-journal` 的「持續性狀況只記錄狀態轉換」要防的情形，
  只是先前只在自動檢查的路徑上實作，漏了這兩個入口。

  使用者另外看到的是 `CHANNEL_TRACKING_UNAVAILABLE:missing_key` 這個內部字串，
  而非可讀的說明 —— 同樣源於缺少前置檢查而直接落到通用的錯誤回報。

  修法三道：兩個模擬入口加上與自動檢查同一條的前置檢查（走 `journalTrackingStatus`
  故只記一次轉換）；迴圈中途才停擺時中止整輪而不逐頻道入帳；中途停擺且
  一部都沒加到時說出原因，不再誤導為「未能取得任何頻道的影片」。
  並加回歸測試釘住「一次動作走訪 20 個頻道只寫一筆」。

  **第一次重測仍失敗，但原因不在程式碼。** `publish_all.ps1` 的順序是
  「[1/3] 打包 → [2/3] 提交 → [3/3] 發布」—— **打包先於提交**。v1.0.94 的
  發布commit 確實含此修正（發布腳本的 `git add .` 把當時正在編輯的檔案掃了
  進去），但該版的 APK 是在修正存檔**之前**就打好的，故安裝包沒有。

  這是一條值得記住的流程事實：發布腳本掃進來的編輯**不會**出現在該版的
  安裝包裡，只會出現在 git 歷史中。tasks 裡「提交與進版期間 MUST NOT 併行
  執行發布腳本」的理由因此比原本寫的更強 —— 不只是可能掃到改到一半的檔案，
  還會讓 commit 與安裝包的內容對不起來。

  v1.0.95 重測：前三步皆通過（可讀說明、只多一筆、連按不再增加），
  但**第四步「填回金鑰後多一筆已恢復」沒有發生**。

  根因是恢復的入帳只掛在手動檢查那條路徑上：`blockSimulationIfTrackingUnavailable`
  在狀態正常時直接返回而未入帳，故全程使用模擬測試時「已恢復」永遠不會寫入。
  更根本的是，**填回金鑰而不做任何檢查**時也不會有人去評估狀態 ——
  而規格要求「停了多久」須能自日誌讀出，那就不該倚賴使用者剛好按了某個按鈕。

  修法兩道：模擬入口在狀態正常時也走一次入帳；並新增 `watch(trackingStatus)`
  使狀態一改變就記錄，涵蓋「填回金鑰未檢查」與「配額於午夜自行重置」
  這兩個使用者完全沒有操作的恢復時刻。兩者與既有的明確呼叫並存不衝突 ——
  `decideJournalTransition` 對相同狀態一律回傳 `none`。分工是
  **watch 捕捉轉換、明確呼叫捕捉首次觀測**（app 啟動時本來就停擺的情形，
  狀態從頭到尾沒變過，watch 不會觸發）。

  **v1.0.96／97 重新驗證通過**：未設金鑰時只寫一筆、連按不再增加，
  填回金鑰後不需按任何檢查即立刻寫入「頻道追蹤（已恢復）」——
  即 `watch(trackingStatus)` 如預期捕捉到使用者完全沒有操作的恢復時刻。
- [x] 5.7 【實作中發現，原任務未涵蓋】人工驗證【遮罩不影響錯誤分類】：`fields` 是否會一併裁剪**錯誤回應**的 body 未經證實，而 `classifyApiError` 正是靠 body 內的 `quotaExceeded`／`accessNotConfigured` 區分「配額耗盡」與「金鑰無效」—— 若錯誤 body 被裁剪，兩者都會退化為 `other`，使 `api-only-channel-tracking` 明訂的抑制與持續狀態指示失效。完成方式：於實機填入一把無效金鑰後手動檢查，確認頻道管理介面顯示的是「金鑰無效」而非泛用的擷取失敗；若確認被裁剪，則自四個遮罩改為只對成功路徑套用（或移除遮罩並於本檔記錄取捨）

  **通過。實測推翻了原本的疑慮 —— `fields` 不會裁剪錯誤回應。**

  v1.0.97 於實機填入無效金鑰後，錯誤紀錄中可見完整的 Google 錯誤 body
  （含 `"reason": "API_KEY_INVALID"` 與 `"status": "INVALID_ARGUMENT"`），
  而 `classifyApiError` 據此正確判為 `key` 而非泛用的 `other` ——
  入帳的文案是 `describeApiKeyRejected()` 的專屬措辭，非泛用失敗。

  這代表 `api-only-channel-tracking` 明訂的「配額耗盡與金鑰無效須可區分」
  與各自的抑制機制，皆未因欄位遮罩而受損。原本擔心的失效情境不成立，
  故四個遮罩維持現狀，不需改為只對成功路徑套用。

  附帶觀察：一次檢查約產生兩筆紀錄（第一個頻道的原始失敗 + 一筆
  「頻道追蹤（已停止）」），同輪其餘頻道由抑制擋下而不再逐一入帳，
  與「持續性狀況只記錄狀態轉換」的設計一致。
