...
- [x] 1.6 人工驗證【頭像】：於實機確認兩個既有頻道（名稱正常、頭像空白）在啟動修復後顯示為真實頻道頭像；並確認未設金鑰時呈現中性佔位而非 YouTube 標誌

  實測確認既有頻道於啟動後顯示真實頻道頭像，通用標誌已不再出現。
...
- [x] 3.1 卡片由雙行改為四行：第一行頭像／名稱／開關／取消追蹤；第二行發布時間；第三行最新影片標題；第四行關鍵字入口與測試按鈕；完成方式：`npx vue-tsc --noEmit` 通過，由 3.5 人工驗證

  **初版做成三行（時間與標題併於第二行），實機驗證後改為四行。**
  使用者回報標題仍被截斷至六至八個字 —— 時間佔去約二十個字元的固定寬度，
  而標題才是判斷「這支要不要看」的依據。已同步修改 `channel-ui-layout`
  的 delta（`Three-line` → `Four-line Card Structure`，上限由第四行改為第五行）
  而非繞過該條要求。
...
- [ ] 3.5 人工驗證【頻道彈窗】：四行版面在手機寬度下不破、無橫向捲軸、標題較先前有更多寬度；關鍵字入口與測試按鈕仍可點；取消追蹤按鈕外觀未變且與開關間距足夠
...
- [x] 4.5 人工驗證【主佇列】：**逐一確認四處相鄰的兩顆 `×` 在實機上分得出來**；播放、上傳、移除、刪檔各自可正確觸發；列高與觸控命中率未劣化

  實測確認四處相鄰的兩顆 `×` 在實機上可區分，播放、上傳、移除、刪檔
  皆可正確觸發。這是本 change 唯一「看錯就回不來」的一關。

  **後續實測回報兩個問題，皆已修正（v1.0.98）：**

  1. **子項那一列的最右側刪檔鍵被擠出卡片邊界。** 該列要放品質標籤、檔案大小
     與四顆字元按鈕，內容約需 270px；而 360px 的畫面扣掉頻道 > 播放清單 > 子項
     三層巢狀縮排後只剩約 220px。按鈕列本就是 `flex-shrink: 0`（§0 紅線 2 的
     要求，不能為了塞進去而縮小），於是超出的部分直接溢出到卡片外。改為
     `.success-action` 允許整列換行、左側資訊可收縮先讓出空間 —— 觸控範圍
     一顆都沒縮小。

  2. **子項的「清除卡片紀錄」不受「清除列表 - 單一」防呆開關約束。**
     這是既有缺陷而非本 change 造成：單一任務卡片走 `removeTask` 會問，
     頻道 > 播放清單 > 子項走的 `removeSubTask` 卻直接清掉，同一個動作在
     兩個畫面行為不一致，而使用者在偏好設定裡明明把開關打開了。已比照
     `removeTask` 接上 `confirmClearSingle`。
